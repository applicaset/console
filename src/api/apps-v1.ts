...
export async function replaceDeployment(axios: Axios, clusterName: string, namespaceName: string, name: string, body: Deployment) {
  await replaceByNamespaceAPIVersionKindName(axios, clusterName, namespaceName, "apps/v1", "Deployment", name,body);
}
...
export async function replaceStatefulSet(axios: Axios, clusterName: string, namespaceName: string, name: string, body: StatefulSet) {
  await replaceByNamespaceAPIVersionKindName(axios, clusterName, namespaceName, "apps/v1", "StatefulSet", name,body);
}

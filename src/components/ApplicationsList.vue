        ...
        <v-col cols="12" v-for="item in items" :key="item.raw">
          <v-card :title="item.raw">
            <v-card-text>
              <v-list>
                <v-list-item
                  v-for="workload in getApplicationWorkloads(clusterName,namespaceName, item.raw)"
                  :key="workload.metadata.uid"
                  :title="workload.metadata.labels['app.kubernetes.io/name'] || workload.metadata.name"

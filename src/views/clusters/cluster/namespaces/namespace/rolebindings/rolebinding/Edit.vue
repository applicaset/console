<template>
  <v-container>
    <v-row>
      <v-col>
        <h2 class="text-h2">Edit Role Binding: {{ roleBindingName }}</h2>
      </v-col>
    </v-row>
    <v-row>
      <v-col>
...
const roleBinding = dataStore.getResource(
  clusterName,
  namespaceName,
...
async function updateRoleBinding() {
  ...
  try {
    await replaceRoleBinding(
      axios,
      clusterName,
      namespaceName,
      roleBindingName,
      yaml.load(roleBindingYaml.value) as RoleBinding,
    );
  } finally {

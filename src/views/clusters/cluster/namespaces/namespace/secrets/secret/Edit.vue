      ...
      <v-col>
        <v-form @submit.prevent="updateSecret">
          <v-card>
            ...
            <v-card-actions>
              <v-btn type="submit" variant="tonal" color="primary" :loading="updatingSecret">Update</v-btn>
            </v-card-actions>
          </v-card>
        </v-form>
      </v-col>
...
const secret = dataStore.getResource(clusterName, namespaceName, "v1", "Secret", secretName);
...
async function updateSecret() {
  ...
  try {
    await replaceSecret(axios, clusterName, namespaceName, secretName, yaml.load(secretYaml.value) as Secret);
  } finally {

      ...
      <v-col>
        <v-card>
          <v-data-table-virtual
            :headers="headers"
            :items="getNamespacedList(clusterName,namespaceName,'networking.k8s.io/v1','Ingress') as Ingress[]"
            multi-sort
          >
            ...
            <template #item._actions="{item}">
              <v-menu>
                ...
                <v-list>
                  <v-list-item
                    ...
                    title="Edit"
                    :to="{name:'IngressEdit', params:{ingressName:item.metadata.name}}"
                  />
              ...
              >
                <v-card title="Delete Confirmation">
                  <v-card-text>
                    Remove Ingress <b>{{ item?.metadata.name }}</b> from namespace <b>{{ item?.metadata.namespace }}</b>?
                  </v-card-text>
                  <v-card-actions class="justify-end">
                    ...
                    <v-btn color="error" :loading="deleting[item.metadata.name]"
                           @click="doDeleteIngress(item.metadata.name)">Remove
                    </v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>

      ...
      <v-col>
        <v-form @submit.prevent="updateCronJob">
          <v-card>
            ...
            <v-card-actions>
              <v-btn type="submit" variant="tonal" color="primary" :loading="updatingCronJob">Update</v-btn>
            </v-card-actions>
          </v-card>
        </v-form>
      </v-col>
...
const cronJob = dataStore.getResource(clusterName, namespaceName, "batch/v1", "CronJob", cronJobName);
...
async function updateCronJob() {
  ...
  try {
    await replaceCronJob(axios, clusterName, namespaceName, cronJobName, yaml.load(cronJobYaml.value) as CronJob);
  } finally {

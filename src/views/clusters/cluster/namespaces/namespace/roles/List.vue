      ...
      <v-col>
        <v-card>
          <v-data-table-virtual
            :headers="headers"
            :items="getNamespacedList(clusterName,namespaceName,'rbac.authorization.k8s.io/v1','Role') as Role[]"
            multi-sort
          >
            ...
            <template #item._actions="{item}">
              <v-menu>
                ...
                <v-list>
                  <v-list-item
                    ...
                    title="Edit"
                    :to="{name:'RoleEdit', params:{roleName:item.metadata.name}}"
                  />
              ...
              >
                <v-card title="Delete Confirmation">
                  <v-card-text>
                    Remove Role <b>{{ item?.metadata.name }}</b> from namespace <b>{{ item?.metadata.namespace }}</b>?
                  </v-card-text>
                  <v-card-actions class="justify-end">
                    ...
                    <v-btn color="error" :loading="deleting[item.metadata.name]"
                           @click="doDeleteRole(item.metadata.name)">Remove
                    </v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
...
async function doDeleteRole(name: string) {
  ...
  try {
    await deleteRole(axios, clusterName, namespaceName, name);
